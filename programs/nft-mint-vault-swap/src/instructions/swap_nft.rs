...
use crate::{common::ErrorCode, AssetLocker, ProtocolConfig, CONFIG_SEED, LOCKER_SEED, VAULT_SEED};
...
pub struct SwapNft<'info> {
  ...
  #[account(
      ...
      bump,
      close = old_owner,
      constraint = asset_locker.owner == old_owner.key() @ErrorCode::Unauthorized,
      constraint = asset_locker.asset == asset.key() @ErrorCode::InvalidLocker,
  )]
...
impl<'info> SwapNft<'info> {
  pub fn handler(&mut self, bumps: SwapNftBumps) -> Result<()> {
  ...
  fn tranfer_sol_to_old_owner(&mut self) -> Result<()> {
      ...
      system_program::transfer(ctx, self.asset_locker.lamports)?;
      Ok(())
  }
